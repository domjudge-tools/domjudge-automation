...
export class DomjudgeService {
   ...
   */
  async getOrganizations(): Promise<Map<string, string | number>> {
    try {
      const response = await this.api.get<Organization[]>(
        `/api/v4/contests/${this.contestId}/organizations`
      );
      const orgMap = new Map<string, string | number>();
   ...
   */
  async getTeams(): Promise<Map<string, number>> {
    try {
   ...
   */
  async getUsers(): Promise<Map<string, number>> {
    try {
      const response = await this.api.get<User[]>(`/api/v4/users`);
  ...
  async createUser(userData: User): Promise<User> {
    try {
      const response = await this.api.post<User>(`/api/v4/users`, userData);
   ...
   */
  async createOrGetOrganization(
    uniName: string,
    existingOrgs: Map<string, string | number>
